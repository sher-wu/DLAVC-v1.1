...
class StyleLoss3d(nn.Module):
    ...
    def __init__(self):
        super(StyleLoss3d, self).__init__()
...
class PerceptualLoss3d(nn.Module):
    ...
    def forward(self, model_output, target):
        ...
        for frame_idx in range(target.shape[1]):
            loss_item.append(
                self.content_loss(model_output[:, frame_idx], target[:, frame_idx])
            )
        mean_loss = torch.stack(loss_item, dim=0).mean(dim=0)
        return mean_loss


# Based on https://github.com/knazeri/edge-connect/blob/master/src/loss.py
class AdversarialLoss3d(nn.Module):
    r"""
    Adversarial loss
    ...
    def __init__(self, type='nsgan', target_real_label=1.0, target_fake_label=0.0):
        ...
        """
        super(AdversarialLoss3d, self).__init__()
