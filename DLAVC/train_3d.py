...
from util.loss3d import StyleLoss3d, PerceptualLoss3d, AdversarialLoss3d
from util.utils import load_state, log_train_data, sample_3d, save_state
from data.data_loader import My3dDataset, create_iterator
...
def set_parser():
    ...
    parser.add_argument('--train_fake_path', default='/data2/wn/Video_dataset/train/frame', help='')
    ...
    parser.add_argument('--test_fake_path', default='/data2/wn/Video_dataset/val/frame', help='')
    parser.add_argument('--board_path', default='./board_3d/', help='')
    parser.add_argument('--log_dir', default='./log_3d/', help='')
    parser.add_argument('--save_path', default='./checkpoint_3d/', help='')
...
if __name__ == '__main__':
    ...
    netG = torch.nn.DataParallel(TemporalConstraintG())
    ...
    L1Loss = nn.L1Loss()
    PercepLoss = PerceptualLoss3d()
    StyleLoss = StyleLoss3d()
    GANLoss = AdversarialLoss3d()
    ...
    train_set = My3dDataset(config.train_kera_path, config.train_fake_path, config.train_gt_path, config.img_size)
    test_set = My3dDataset(config.test_kera_path, config.test_fake_path, config.test_gt_path, config.img_size)
    ...
    for epoch in range(epoch, config.epoch + 1):
        for iteration, [kera, fake, gt, ref_keras, ref_gts] in enumerate(training_data_loader):
            ...
            pred_refine = netD(refine)
            loss_D_fake = GANLoss(pred_refine, False, True)
            pred_gt = netD(gts)
            ...
            loss_G_gan = GANLoss(pred_refine, True, False) * config.Ganlamb
            loss_G_l1 = L1Loss(refine, gts) * config.L1lamb
            loss_G_percep = PercepLoss(refine, gts) * config.Perceplamb
            loss_G_style = StyleLoss(refine, gts) * config.Stylelamb
            loss_G = loss_G_gan + loss_G_l1 + loss_G_percep + loss_G_style
            ...
            if iteration % config.sample_freq == 0:
                ...
                sample_3d(count, sample_train_iterator, netG.eval(), writer, "train")
                sample_3d(count, sample_test_iterator, netG.eval(), writer, "test")

            print("===> Epoch[{}]({}/{}): Loss_D: {:.4f} LossD_Fake: {:.4f} LossD_Real: {:.4f}  "
                  "Loss_G: {:.4f} LossG_Adv: {:.4f} LossG_L1: {:.4f} "
                  "LossG_Style {:.4f} LossG_Percep {:.4f}".format(
                epoch, iteration, len(training_data_loader), loss_D, loss_D_fake, loss_D_real, loss_G,
                loss_G_gan, loss_G_l1, loss_G_style, loss_G_percep))
